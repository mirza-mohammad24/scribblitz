...
export default function Home() {
  ...
  // Identify if we are actively in the game arena (not home, not lobby)
  const isArena = gameState !== null && gameState !== GameState.LOBBY;
  ...
  return (
    ...
    <main className="flex h-dvh flex-col items-center p-2 lg:p-4 transition-colors duration-300 w-full max-w-400 mx-auto min-h-0 bg-transparent dark:bg-discord-main">
      ...
      {/* GLOBAL HEADER: Appears on Splash, Lobby, and Game Arena, Hidden on mobile during active gameplay to 
      maximize screen real estate */}
      <div
        className={`w-full justify-between items-center mb-4 z-10 shrink-0 ${isArena ? 'hidden md:flex' : 'flex'}`}
      >
